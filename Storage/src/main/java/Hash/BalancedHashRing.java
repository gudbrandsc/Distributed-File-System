...
public class BalancedHashRing<T> implements HashRing<T> {
    ...
    private TreeMap<BigInteger, HashRingEntry> entryMap = new TreeMap<>();

    public TreeMap<BigInteger, HashRingEntry> getEntryMap() {
        return entryMap;
    }

    public ArrayList<HashRingEntry> getEntryList(){
        ArrayList<HashRingEntry> entries = new ArrayList<>();
    ...
    public HashRingEntry getEntry(BigInteger key){
       return entryMap.get(key);
    }

    public HashRingEntry getEntryById(int nodeid){
        for(HashRingEntry entry : entryMap.values()){
     ...
     * @param position place in the hash space
     */
    public synchronized boolean removeRingEntry(BigInteger position){
